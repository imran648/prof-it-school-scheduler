...
const Finances = () => {
  ...
  useEffect(() => {
    if (selectedGroupId) {
      const groupPayments = getGroupPayments(selectedGroupId);
      
      // Фильтруем платежи по выбранной вкладке
      let filtered;
      if (selectedTab === 'all') {
        filtered = groupPayments;
      } else {
        filtered = groupPayments.filter(payment => payment.status === selectedTab);
      }
  ...
  }, [selectedGroupId, selectedTab, payments]);
